-- =============================================================================
-- 003: Vincular turnos por DNI + corte de horarios en minutos
-- =============================================================================

ALTER TABLE public.system_settings
  ADD COLUMN IF NOT EXISTS booking_cutoff_minutes INTEGER NOT NULL DEFAULT 30;

COMMENT ON COLUMN public.system_settings.booking_cutoff_minutes IS
  'Minutos antes del inicio del turno en los que el horario deja de mostrarse/aceptarse online.';

-- Default operativo: 30 minutos. Resetear horas para no anular el corte en minutos.
UPDATE public.system_settings
SET booking_cutoff_minutes = COALESCE(NULLIF(booking_cutoff_minutes, 0), 30),
    min_advance_hours = 0;
...
BEGIN
  ...
  -- Vincular por DNI (identificador del paciente). Si existe, usa esa ficha
  -- aunque el nombre no coincida. Si no existe y auto_create está activo, crea paciente.
  ...
  IF v_patient_id IS NOT NULL THEN
    -- Actualizar teléfono para poder contactar por WhatsApp
  ...
  INSERT INTO public.appointments (
    ...
    trim(p_first_name), trim(p_last_name), v_dni,
