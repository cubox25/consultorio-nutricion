-- El DNI es la identidad del paciente.
-- Si ya existe una ficha con ese DNI, el turno se agenda con el nombre de esa ficha
-- (no con el nombre que tipeó en el formulario).
...
BEGIN
  ...
  -- Identidad = DNI. Si ya hay ficha, usamos ese nombre (no el del formulario).
  SELECT id
  INTO v_patient_id
