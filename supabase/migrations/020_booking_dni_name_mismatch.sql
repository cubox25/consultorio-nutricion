-- Si alguien reserva con un DNI que ya existe pero otro nombre,
-- no se pega a esa ficha (evita "un paciente duplicado / otro que no aparece").

CREATE OR REPLACE FUNCTION public._norm_person_name(p TEXT)
RETURNS TEXT
LANGUAGE sql
IMMUTABLE
AS $$
...
BEGIN
  ...
  v_guest_first := trim(p_first_name);
  v_guest_last := trim(p_last_name);
  ...
  INTO v_patient_id, v_patient_first, v_patient_last
  ...
  IF v_patient_id IS NOT NULL THEN
    IF public._norm_person_name(v_patient_first) <> public._norm_person_name(v_guest_first)
       OR public._norm_person_name(v_patient_last) <> public._norm_person_name(v_guest_last)
    THEN
      RAISE EXCEPTION
        'Este DNI ya está registrado a nombre de % %. Si no sos esa persona, revisá el número de DNI.',
        v_patient_first,
        v_patient_last;
    END IF;
  ...
  INSERT INTO public.appointments (
    ...
    v_guest_first, v_guest_last, v_dni,
  ...
  RETURN jsonb_build_object(
    'id', v_id,
    'first_name', v_guest_first,
    'last_name', v_guest_last
  );
EXCEPTION
